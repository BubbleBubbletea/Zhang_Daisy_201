...
win_spread <- spread[spread > 0]
largest_win <- max(win_spread, na.rm = TRUE)  # if Bears never won
win_index <- which(spread == largest_win)
cat("The largest win spread is", largest_win,", when the Bears scored", bears[win_index], 
     "points and the Opponent scored", opponent[win_index], "points.\n")
...
new_row <- c(37, 17)
new_row2<- c(9, 17)
game_score <- rbind(game_score, new_row)
game_score <- rbind(game_score, new_row2)
print(game_score)
...
```{r}
first_win <- which(game_score[, "win"] == 1)[1]
if (!is.na(first_win)) {
  losses_before_win <- first_win - 1
} else {
  losses_before_win <- nrow(game_score)  
}
cat("The Bears lost", losses_before_win, "games before their first win.\n")
```
...
```{r}
number_of_factors <- function(myint){
  num_fac <- 0
  for(i in 1:myint){
    if(myint %% i == 0){
      num_fac <- num_fac + 1
...
```{r}
area_rectangle <- function(len=48, wid=48){
  area_inch <- len * wid
  area_ft <- (len/12) * (wid/12)
  return(list(square_inches = area_inch, square_feet = area_ft))
}
area_result <- area_rectangle(102, 60)
cat("The area in square inches is", area_result$square_inches, ".\n")
cat("The area in square feet is", area_result$square_feet, ".\n")
```
...
```{r}
is_val_vec <- function(val, vec){
  return(val %in% vec)
}
result <- is_val_vec(value, check)
print(result)
```
...
```{r}
negatives_to_NA <- function(mat) {
  mat[mat < 0] <- NA  
  return(mat)
}
new_x <- negatives_to_NA(x)
print(new_x)
```
